...
from geoopt import Euclidean, ManifoldParameter, ManifoldTensor
...
class Trainer(object):
    ...
    def __init__(self, args, task, model, criterion, quantizer=None):
        ...
        if self.cuda and self.data_parallel_world_size > 1:
            self._grad_norm_buf = torch.cuda.FloatTensor(self.data_parallel_world_size) # torch.cuda.DoubleTensor(self.data_parallel_world_size)
        else:
    ...
    def _build_optimizer(self):
        ...
        for name, p in chain(self.model.named_parameters(), self.criterion.named_parameters()):
            ...
            if not _manifold_name in params_dict:
                ...
                params_dict[_manifold_name] = dict(params=[], lr_rectifier=ref_grad.reciprocal().item() * coef)
    ...
    def _check_grad_norms(self, grad_norm):
        ...
        if self._grad_norm_buf is not None:
            ...
            if not (self._grad_norm_buf == self._grad_norm_buf[0]).all():
                print(self._grad_norm_buf)
                print(self._grad_norm_buf == self._grad_norm_buf[0])
                raise RuntimeError(
